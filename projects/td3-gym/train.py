#!/usr/bin/env python3
"""TD3 (Twin Delayed DDPG) on Gymnasium continuous-control envs.
...
parser.add_argument("--out_dir", default="/content/td3-output")
...
log_print(f"=== TD3 {args.env} | device={device} | {datetime.now()} ===")
...
class Critic(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        # Q2 architecture (separate parameters)
        self.fc4 = nn.Linear(s_dim + a_dim, 256)
...
def select_action(state, add_noise=True):
    ...
    if add_noise:
        a += np.random.normal(0, args.exploration_noise, size=action_dim)
...
def plot_progress():
    ...
    # Reward
    ax = axes[0, 0]
    ax.plot(eps, rewards, "b-", alpha=0.5, linewidth=0.8)
    if len(eps) >= 10:
        window = min(10, len(rewards))
        smooth = np.convolve(rewards, np.ones(window) / window, mode="valid")
        ax.plot(eps[window - 1:], smooth, "b-", linewidth=1.5, label=f"MA{window}")
    ax.set_xlabel("Episode"); ax.set_ylabel("Total Reward")
    ax.set_title(f"{args.env} — TD3 Episode Reward")
    ...
    if metrics["eval_episodes"]:
        ...
        axes[0, 1].set_title("TD3 Evaluation (no noise)")
    ...
    if actor_eps:
        ...
        axes[1, 0].set_title("Actor & Critic Loss"); axes[1, 0].legend()
...
for episode in range(1, args.episodes + 1):
    ...
    for t in range(args.steps_per_episode):
        ...
        if len(buffer) > args.batch_size:
            ...
            with torch.no_grad():
                noise = (torch.randn_like(a) * args.policy_noise).clamp(
                    -args.noise_clip, args.noise_clip
                )
                target_a = (target_actor(ns) + noise).clamp(-max_action, max_action)
                q1_targ, q2_targ = target_critic(ns, target_a)
                target_q = r + args.gamma * torch.min(q1_targ, q2_targ) * (1 - d)
            ...
            if n_updates % args.policy_delay == 0:
                pred_a = actor(s)
                actor_loss = -critic.q1(s, pred_a).mean()
                actor_opt.zero_grad()
                actor_loss.backward()
    ...
    log_print(
        f"[ep {episode:4d}] reward={ep_reward:8.2f}  steps={t + 1:3d}  "
