...
parser.add_argument("--envs", nargs="+",
                    default=["HalfCheetah-v5", "Hopper-v5", "Walker2d-v5"])
parser.add_argument("--out_dir", default="/content/ppo-mujoco-output")
...
csv_writer.writerow(["iteration", "env", "total_steps", "mean_reward", "avg10_reward",
                     "eval_reward", "pg_loss", "vf_loss", "entropy", "clip_frac", "lr", "elapsed_s"])
csv_file.flush()
...
def plot_curves(env_name, history, out_dir):
    ...
    ax = axes[0, 1]
    ax.plot(iters, [h["pg_loss"] for h in history], color="crimson", linewidth=1.2, label="Policy loss")
    ax.plot(iters, [h["vf_loss"] for h in history], color="royalblue", linewidth=1.2, label="Value loss")
    ax.set_xlabel("Iteration"); ax.set_ylabel("Loss"); ax.set_title("Losses")
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    ax = axes[1, 0]
    ax.plot(iters, [h["entropy"] for h in history], color="mediumseagreen", linewidth=1.5)
    ax.set_xlabel("Iteration"); ax.set_ylabel("Entropy"); ax.set_title("Policy Entropy")
    ax.grid(True, alpha=0.3)

    ax = axes[1, 1]
    ax.plot(iters, [h["clip_frac"] for h in history], color="darkviolet", linewidth=1.5)
    ax.axhline(y=0.2, color="gray", linestyle="--", alpha=0.5, label="clip_coef=0.2")
    ...
    plt.tight_layout()
    fig.savefig(f"{out_dir}/pngs/{env_name}_curves.png", dpi=120, bbox_inches="tight")
    plt.close(fig)
...
def plot_comparison(all_env_histories, out_dir):
    ...
    for idx, (name, history) in enumerate(sorted(all_env_histories.items())):
        evals = [(h["iteration"], h["eval_reward"]) for h in history if h.get("eval_reward") is not None]
        if evals:
            xs, ys = zip(*evals)
            ax.plot(xs, ys, color=colors[idx], linewidth=1.8, label=name, marker="o", markersize=3)
    ax.set_xlabel("Iteration"); ax.set_ylabel("Eval Reward")
    ax.set_title("PPO MuJoCo — Evaluation Comparison")
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def evaluate(agent, env_id, n_episodes=10):
    ...
    for _ in range(n_episodes):
        ...
        while not done:
            ...
            obs, reward, terminated, truncated, _ = eval_env.step(action.squeeze(0).numpy())
            ep_reward += reward
...
log(f"=== PPO MuJoCo | device={device} | {datetime.now()} ===")
...
for env_idx, env_id in enumerate(args.envs):
    ...
    cfg = load_config(env_id, args.config_dir)
    log(f"Config: network={cfg['network']} timesteps={cfg['total_timesteps']} "
        f"obs_dim={cfg['obs_dim']} n_actions={cfg['n_actions']} lr={cfg['lr']}")

    num_envs = cfg["num_envs"]; num_steps = cfg["num_steps"]
    total_timesteps = cfg["total_timesteps"]
    n_iterations = total_timesteps // (num_envs * num_steps)

    envs = make_env(env_id, num_envs=num_envs, seed=cfg["seed"])
    net = NETWORK_REGISTRY[cfg["network"]](cfg["obs_dim"], cfg["n_actions"])
    agent = PPOAgent(network=net, lr=cfg["lr"], gamma=cfg["gamma"],
                     gae_lambda=cfg["gae_lambda"], clip_coef=cfg["clip_coef"],
                     ent_coef=cfg["ent_coef"], vf_coef=cfg["vf_coef"],
                     max_grad_norm=cfg["max_grad_norm"], n_epochs=cfg["n_epochs"],
                     n_minibatches=cfg["n_minibatches"], device=str(device))

    history = []; reward_buffer = deque(maxlen=10)
    best_eval = -float("inf"); start_time = time.time()
    ...
    for iteration in range(1, n_iterations + 1):
        rollouts = {
            "obs": torch.zeros(num_steps, num_envs, cfg["obs_dim"]),
            "actions": torch.zeros(num_steps, num_envs, cfg["n_actions"]),
        ...
        advantages, returns = agent.compute_returns(
            obs, next_done, rollouts["rewards"], rollouts["dones"], rollouts["values"])
        rollouts["advantages"] = advantages; rollouts["returns"] = returns
        ...
        history.append({"iteration": iteration, "mean_reward": mean_reward,
                        "avg10_reward": avg10, "eval_reward": None,
                        "pg_loss": metrics["pg_loss"], "vf_loss": metrics["vf_loss"],
                        "entropy": metrics["entropy"], "clip_frac": metrics["clip_frac"]})

        csv_writer.writerow([iteration, env_id, total_steps,
                             round(mean_reward, 2), round(avg10, 2), None,
                             round(metrics["pg_loss"], 6), round(metrics["vf_loss"], 6),
                             round(metrics["entropy"], 4), round(metrics["clip_frac"], 4),
                             cfg["lr"], round(elapsed, 1)])
        csv_file.flush()

        log(f"iter {iteration:3d}/{n_iterations} | {env_id:20s} "
            f"reward={mean_reward:8.1f} avg10={avg10:8.1f} | "
            f"pg_loss={metrics['pg_loss']:.4f} vf_loss={metrics['vf_loss']:.4f} | "
            f"ent={metrics['entropy']:.2f} clip={metrics['clip_frac']:.3f} | steps={total_steps}")
        ...
        if iteration % cfg["eval_interval"] == 0:
            ...
            log(f"  EVAL: mean={eval_mean:.1f} +- {eval_std:.1f}")
        ...
        if iteration % cfg["plot_interval"] == 0:
            ...
            all_histories[env_id] = history
            plot_comparison(all_histories, args.out_dir)
            subprocess.run(["tar", "-czf", "/content/ppo-mujoco-output.tar.gz",
                            "-C", "/content", "ppo-mujoco-output"], capture_output=True)
...
for env_name, hist in all_histories.items():
    ...
    summary[env_name] = {"iterations": len(hist),
...
for env_name, s in sorted(summary.items()):
    log(f"  {env_name:30s}  best_eval={s['best_eval_reward']:8.1f}  final_avg10={s['final_avg10_reward']:8.1f}")
