"""Combined eval + watchdog: runs evaluation while keeping WebSocket alive.

Replaces check_and_eval.py for deployment. Prints heartbeats every 15s during
model loading and every 5 examples during eval — real TCP payload resets NAT timers.

Upload once, then launch immediately after bg_launch.py:
    colab exec -s <name> -f eval_and_watch.py --timeout 600

If training isn't done yet, waits (with heartbeats) then evaluates.
Session survives because WebSocket is active the entire time.
"""
import os, sys, subprocess, shutil, json, re, sqlite3, time
from concurrent.futures import ThreadPoolExecutor, TimeoutError as FuturesTimeoutError
from datetime import datetime, timezone

PROJECT_DIR = "/content/text2sql_finetune"
OUTPUT_DIR = "/content/text2sql-finetune-output"
MODEL_NAME = "Qwen/Qwen3-0.6B"
SQL_TIMEOUT = 5
MAX_WAIT = 300  # max seconds to wait for training
HB_INTERVAL = 15  # heartbeat every 15s

os.makedirs(f"{OUTPUT_DIR}/logs", exist_ok=True)


def ts():
    return datetime.now(timezone.utc).strftime("%H:%M:%S")


def heartbeat(msg):
    print(f"[{ts()}] EVAL-WD: {msg}", flush=True)


# ── Wait for training to complete ───────────────────────
log_path = f"{PROJECT_DIR}/logs/train.log"
adapter_config = f"{PROJECT_DIR}/lora_weights/adapter_config.json"
adapter_dir = f"{PROJECT_DIR}/lora_weights"
waited = 0

heartbeat("starting — checking training status")
while not os.path.exists(adapter_config) and waited < MAX_WAIT:
    tail = "(no log)"
    if os.path.exists(log_path):
        try:
            with open(log_path) as f:
                lines = f.readlines()
                tail = lines[-1].strip()[-150:] if lines else "(empty)"
        except Exception:
            pass
    heartbeat(f"waiting for training... ({waited}s) log: {tail}")
    time.sleep(HB_INTERVAL)
    waited += HB_INTERVAL

if not os.path.exists(adapter_config):
    heartbeat(f"FATAL: training not complete after {MAX_WAIT}s")
    sys.exit(1)

heartbeat("training complete — loading model for eval")

# ── Imports (after wait, to avoid blocking training) ────
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer
from peft import PeftModel

device = torch.device("cuda")
tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, trust_remote_code=True)
heartbeat("tokenizer loaded")

base_model = AutoModelForCausalLM.from_pretrained(
    MODEL_NAME, torch_dtype=torch.bfloat16, device_map="auto", trust_remote_code=True,
)
heartbeat("base model loaded")

eval_model = PeftModel.from_pretrained(base_model, adapter_dir)
eval_model.eval()
heartbeat("LoRA weights loaded — starting evaluation")

# ── Load test data ──────────────────────────────────────
test_path = f"{PROJECT_DIR}/data/test.pt"
if not os.path.exists(test_path):
    heartbeat(f"FATAL: test data not found: {test_path}")
    sys.exit(1)
test_examples = torch.load(test_path, weights_only=False)
heartbeat(f"loaded {len(test_examples)} test examples")


# ── SQL helpers ─────────────────────────────────────────
def extract_sql(text):
    pattern = r"<\|im_start\|>assistant\n(.*?)(?:<\|im_end\|>|$)"
    matches = re.findall(pattern, text, re.DOTALL)
    raw = matches[-1].strip() if matches else ""
    if not raw:
        idx = text.rfind("assistant\n")
        raw = text[idx + len("assistant\n"):].strip() if idx != -1 else text.strip()
    raw = re.sub(r"<think>.*?</think>\s*", "", raw, flags=re.DOTALL).strip()
    return raw


def parse_create_table(sql):
    return re.findall(r"CREATE\s+TABLE\s+\w+\s*\([^)]+\)", sql, re.IGNORECASE)


def execute_sql(create_tables, query):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    try:
        for ct in create_tables:
            conn.execute(ct)
        conn.commit()
        cursor = conn.execute(query)
        return True, [tuple(row) for row in cursor.fetchall()]
    except Exception as e:
        return False, str(e)
    finally:
        conn.close()


def execute_with_timeout(create_tables, query, timeout=SQL_TIMEOUT):
    with ThreadPoolExecutor(max_workers=1) as executor:
        try:
            return executor.submit(execute_sql, create_tables, query).result(timeout=timeout)
        except FuturesTimeoutError:
            return False, "__TIMEOUT__"


# ── Eval loop ───────────────────────────────────────────
results = []
error_counts = {"syntax": 0, "timeout": 0, "wrong_result": 0, "other": 0}
exec_matches = 0
exact_matches = 0
eval_start = time.time()

eval_log_path = f"{OUTPUT_DIR}/logs/eval.log"
eval_fh = open(eval_log_path, "w")

for i, ex in enumerate(test_examples):
    input_ids = ex["input_ids"]
    labels = ex["labels"]

    label_start = next((j for j in range(len(labels)) if labels[j] != -100), len(labels))
    prompt_ids = input_ids[:label_start]
    gt_ids = [int(input_ids[j]) for j in range(label_start, len(labels)) if labels[j] != -100]

    gt_sql = tokenizer.decode(gt_ids, skip_special_tokens=True).strip()

    prompt_tensor = input_ids[:label_start].unsqueeze(0).to(device)
    attn_tensor = ex["attention_mask"][:label_start].unsqueeze(0).to(device)

    with torch.no_grad():
        generated = eval_model.generate(
            input_ids=prompt_tensor, attention_mask=attn_tensor,
            max_new_tokens=512, do_sample=False,
            pad_token_id=tokenizer.pad_token_id or tokenizer.eos_token_id,
            eos_token_id=tokenizer.eos_token_id,
        )
    full_output = tokenizer.decode(generated[0], skip_special_tokens=False)
    gen_sql = extract_sql(full_output)

    context_text = tokenizer.decode(input_ids[:label_start].tolist(), skip_special_tokens=True)
    create_tables = parse_create_table(context_text)
    if not create_tables:
        create_tables = parse_create_table(tokenizer.decode(input_ids.tolist(), skip_special_tokens=True))

    exact_match = (gen_sql.strip().lower() == gt_sql.strip().lower())
    exec_match = False
    error_type = None

    if gen_sql:
        success, result = execute_with_timeout(create_tables, gen_sql)
        if success:
            gt_success, gt_result = execute_sql(create_tables, gt_sql)
            if gt_success:
                exec_match = (result == gt_result)
                if not exec_match:
                    error_type = "wrong_result"
            else:
                error_type = "other"
        else:
            error_type = "timeout" if result == "__TIMEOUT__" else "syntax"
    else:
        error_type = "syntax"

    if exec_match:
        exec_matches += 1
    if exact_match:
        exact_matches += 1
    if error_type:
        error_counts[error_type] += 1

    results.append({
        "generated_sql": gen_sql,
        "ground_truth": gt_sql,
        "exact_match": exact_match,
        "exec_match": exec_match,
        "error": error_type,
    })

    eval_fh.write(f"--- {i+1} ---\nGen: {gen_sql[:300]}\nGT: {gt_sql[:300]}\n"
                  f"Exec: {exec_match} Err: {error_type}\n\n")

    # Heartbeat every 5 examples — keeps WebSocket alive
    if (i + 1) % 5 == 0 or i < 3:
        elapsed = time.time() - eval_start
        heartbeat(f"eval {i+1}/{len(test_examples)} | "
                  f"exec_acc={exec_matches/(i+1):.3f} | elapsed={elapsed:.0f}s")

    # Show first 3 raw outputs for debugging
    if i < 3:
        heartbeat(f"  debug[{i}]: gen_sql={repr(gen_sql[:200])}")

eval_fh.close()

# ── Report ──────────────────────────────────────────────
n = len(results)
report = {
    "execution_accuracy": exec_matches / n if n else 0,
    "exact_match_accuracy": exact_matches / n if n else 0,
    "total": n,
    "errors": error_counts,
    "eval_time_s": time.time() - eval_start,
}

os.makedirs(OUTPUT_DIR, exist_ok=True)
with open(f"{OUTPUT_DIR}/eval_report.json", "w") as f:
    json.dump(report, f, indent=2)

# Gather outputs
for src in [f"{OUTPUT_DIR}/logs/eval.log", f"{OUTPUT_DIR}/eval_report.json",
            f"{PROJECT_DIR}/logs/train.log", f"{PROJECT_DIR}/logs/metrics.csv"]:
    if os.path.exists(src):
        dst = os.path.join(OUTPUT_DIR, os.path.basename(src))
        if os.path.abspath(src) != os.path.abspath(dst):
            shutil.copy2(src, dst)

# Tar
subprocess.check_call([
    "tar", "-czf", "/content/text2sql-finetune-output.tar.gz",
    "-C", "/content", "text2sql-finetune-output",
])

total_elapsed = time.time() - eval_start
heartbeat(f"DONE | exec_acc={report['execution_accuracy']:.3f} | "
          f"exact_match={report['exact_match_accuracy']:.3f} | "
          f"errors={error_counts} | eval_time={total_elapsed:.0f}s")
print(f"\nDownload: colab download -s <session> /content/text2sql-finetune-output.tar.gz")
