"""PPO Agent with GAE — CleanRL-style for continuous actions (MuJoCo)."""
import torch
...
class PPOAgent:
    def __init__(
        self, network: nn.Module, lr: float = 3e-4, gamma: float = 0.99,
        gae_lambda: float = 0.95, clip_coef: float = 0.2, ent_coef: float = 0.0,
        vf_coef: float = 0.5, max_grad_norm: float = 0.5, n_epochs: int = 10,
        n_minibatches: int = 32, device: str = "cuda",
    ):
    ...
    def compute_returns(
        ...
        rewards: torch.Tensor, dones: torch.Tensor, values: torch.Tensor,
    ...
    def update(self, rollouts: dict) -> Dict[str, float]:
        num_steps, num_envs = rollouts["actions"].shape[:2]
        batch_size = num_steps * num_envs
        minibatch_size = batch_size // self.n_minibatches

        obs_dim = rollouts["obs"].shape[-1]
        act_dim = rollouts["actions"].shape[-1]

        b_obs = rollouts["obs"].reshape(-1, obs_dim).to(self.device)
        b_actions = rollouts["actions"].reshape(-1, act_dim).to(self.device)
        ...
        metrics_sum = {"pg_loss": 0.0, "vf_loss": 0.0, "entropy": 0.0,
