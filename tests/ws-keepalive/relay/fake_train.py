"""Fake GPU training — 20 minutes, timestamped logging, PID file for watchdogs."""
import time, os, sys
...
log(f"TOTAL_ITERATIONS={TOTAL_ITERATIONS} interval={INTERVAL}s duration={TOTAL_MINUTES}min")
...
for i in range(TOTAL_ITERATIONS):
    ...
    log(f"iter={i+1}/{TOTAL_ITERATIONS} elapsed={elapsed:.0f}s "
        f"deadline={elapsed/60:.1f}min {gpu_status} dt={iter_time:.2f}s")

    if i < TOTAL_ITERATIONS - 1:
        time.sleep(max(0, INTERVAL - iter_time))

log(f"TRAIN_COMPLETE total_elapsed={time.time()-start_time:.0f}s")
